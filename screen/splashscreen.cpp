...
SplashScreen::SplashScreen(const QPixmap &pixmap)
    : QSplashScreen(pixmap), m_percent(0), m_mainWidget(nullptr),
...
void SplashScreen::paintEvent(QPaintEvent *event) {
  ...
  painter.drawRoundedRect(OFFSET_VALUE, this->height() - OFFSET_VALUE,
                          (this->width() - OFFSET_VALUE * 2), BORDER_X_RADIUS,
                          BORDER_Y_RADIUS, SLIDER_HEIGHT);
  ...
  // 绘制消息文本
  {
    ...
    painter.setPen(painterPen);
    painter.drawText(0, this->height() - OFFSET_VALUE * 2, this->width(),
                     OFFSET_VALUE, Qt::AlignCenter, m_message);
  }
  ...
  // 绘制进度条
  {
    ...
    if (m_percent != 0) {
      ...
      painter.setBrush(brush);
      painter.drawRoundedRect(OFFSET_VALUE, this->height() - OFFSET_VALUE,
                              (this->rect().width() - OFFSET_VALUE * 2) *
                                  m_percent / 100,
  ...
  // 绘制进度文本
  {
    ...
    painter.drawText(0, this->height() - OFFSET_VALUE, this->width(),
                     OFFSET_VALUE, Qt::AlignCenter, drawText);
    painter.end();
  }
...
// 开始
void SplashScreen::setStart(QWidget *widget, const QString &title,
                            const QString &logoFile) {
  if (nullptr != widget) {
    m_mainWidget = widget;
    ...
    if (nullptr == m_propertyAnimation) {
      m_propertyAnimation =
          new QPropertyAnimation(m_mainWidget, "windowOpacity");
